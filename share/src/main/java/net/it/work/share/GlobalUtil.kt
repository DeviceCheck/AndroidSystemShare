...
object GlobalUtil {
    ...
    val appPackage: String
        get() = ShareManager.context.packageName
     ...
     */
    val appName: String
        get() = ShareManager.context.resources.getString(ShareManager.context.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = ShareManager.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ShareManager.context.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            ShareManager.context.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(ShareManager.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getString(resId: Int): String {
        return ShareManager.context.resources.getString(resId)
    }
    ...
    fun getDimension(resId: Int): Int {
        return ShareManager.context.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return ShareManager.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = ShareManager.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            ShareManager.context.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
    ...
    fun getAppIcon(): Drawable {
        val packageManager = ShareManager.context.packageManager
